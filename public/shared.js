...
/**
 * Retry wrapper with explicit timeout (default 8s).
 * Used by weather, FX history, and retention modules.
 */
async function fetchWithRetry(url, options = {}, meta = {}) {
    ...
    if (!silent) {
        console.warn(`[fetchWithRetry] ${context} failed after ${retries + 1} attempt(s):`, lastError?.message);
    }
    throw lastError;
}

async function fetchWeatherExtended(lat, lon, options = {}) {
    const silent = options.silent ?? true;
    const params = new URLSearchParams({
        latitude: lat,
        longitude: lon,
    ...
    try {
        const res = await fetchWithRetry(
            `${WEATHER_API}?${params}`,
            {},
            { silent, context: 'weather-extended', timeoutMs: DEFAULT_FETCH_TIMEOUT_MS }
        );
        const data = await res.json();
